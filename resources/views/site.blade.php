		...
		$(document).ready(function(){
			$.ajax({
			  ...
			  success: function(data) {
				var row;
				row = $("<tr><td>" + data.site.nome + "</td><td> " + data.site.url + "</td><td> " + data.site.pm + "</td><td> " + data.site.customer.support_queue + '</td><td> ' + data.site.technology.name + '</td><td> ' + data.site.machine.name + '</td><td> ' + data.site.doc_root + '</td><td> ' + data.site.group + '</td><td> ' + data.site.cms_admin + '</td><td> ' + data.site.cms_pass + '</td><td> ' + data.site.auth_name + '</td><td> ' + data.site.auth_pass + '</td>');
				row.append('<td id="db_host'+data.site.sid+'"></td><td id="db_user'+data.site.sid+'"></td><td id="db_pass'+data.site.sid+'"></td><td id="db_name'+data.site.sid+'"></td>');
				row.append('<td>' + data.site.notes + '</td></tr>');
				$('#table').append(row);
				for(var key in data.site.database) {
					$('#db_host'+data.site.sid).append(data.site.database[key].host + '<br/>');
					$('#db_user'+data.site.sid).append(data.site.database[key].username + '<br/>');
					$('#db_pass'+data.site.sid).append(data.site.database[key].password + '<br/>');
					$('#db_name'+data.site.sid).append(data.site.database[key].db_name + '<br/>');
				}
			  },
			  type: 'GET'
		   });
